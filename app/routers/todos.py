...
def read_todos(skip: int = 0, limit: int = 20, db: Session = Depends(database.get_db)):
    db_todos = todos_controller.get_todos(db, skip, limit)
    return db_todos


@router.get("/{id}", response_model=todo_schema.TodoResponse)
def read_todo(id: str, db: Session = Depends(database.get_db)):
...
def create_todo(todo: todo_schema.TodoCreate, db: Session = Depends(database.get_db)):
    created_todo = todos_controller.create_todo(db, todo)
    return created_todo


@router.put("/{id}", response_model=todo_schema.TodoResponse)
def update_todo(id: str, todo: todo_schema.TodoUpdate, db: Session = Depends(database.get_db)):
...
@router.delete("/{id}")
def delete_todo(id: str, db: Session = Depends(database.get_db)):
